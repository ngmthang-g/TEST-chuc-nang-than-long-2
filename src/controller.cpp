...
private:
    ...
    bool RunSellMacroClick(Account& a, DWORD now) {
        RuntimeState& rt = a.runtime;
        const DWORD delay = rt.sellMacroIndex < 4 ? 350u : (rt.sellMacroIndex == 4 ? 260u : 180u);
        ...
        if (rt.sellMacroIndex == 4) {
            if (!a.bridge.Call(Command::SellNextBagItem, 0, 0, 0, response, error, 2600)) {
            ...
            if (response.resultCode == static_cast<std::int32_t>(ActionResult::NoCandidate)) {
                ...
                rt.status = L"BÁN NỀN • callback " + std::to_wstring(rt.sellMacroRepeatDone) +
                            ...
                            L" • sold verified=" + std::to_wstring(response.value1);
            }
            return true;
        }
