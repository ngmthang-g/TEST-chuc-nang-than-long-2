...
- v1.1.5 repeats unchanged v1.1.4 action layer at NPC 463/Lạc Dương: user reports the same symptom.

### Unknowns
- Whether the old direct/hook invocation causes a re-entrant lifecycle problem before normal Unity Update processing.
- Whether v1.1.6 MainThread-queued event will reproduce the real user click path successfully.
- Exact post-Treatment server/UI transition after a genuinely accepted action.

### Root Cause
**LIKELY but not CONFIRMED:** common external action execution-boundary issue. The symptom survives NPC/map changes and multiple source-correct semantic action attempts. Canonical client bridge docs require mutable actions to execute through `MainThread.Execute(Action)` and later Unity Update rather than direct re-entrant mutation from the Windows message hook.
...
6. v1.1.6: CTS-proven `MainThread.Execute(System.Action)` + queued live UIButton event — BUILD/RUNTIME pending at registry creation.
